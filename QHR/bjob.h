 ...
 * libqhr with `cmake -DWITH_KDE` to use the original KJob implementation.
 */
class QHR_LIBRARY BJob : public QObject
{
    Q_OBJECT
    /*!
     * \brief Error code.
     ...
     *
     * \par Notifier signal
     * \li void result(BJob *job)
     *
     * \sa BJob::errorString, BJob::errorText
     */
     ...
     * associated with the \link BJob::error error\endlink, such as a URL.
     *
     * \par Access functions
     ...
     * \li void result(BJob *job)
     *
     * \sa BJob::error, BJob::errorString
     */
     ...
     *
     * \par Notifier signal
     * \li void percent(BJob *job, unsigned long percent)
     */
     ...
     */
    Q_PROPERTY(QHR::BJob::Capabilities capabilities READ capabilities CONSTANT)
public:
    /*!
     * \brief Error codes.
     */
    ...
    /*!
     * \brief Constructs a new %BJob object with the given \a parent.
     */
    explicit BJob(QObject *parent = nullptr);

    /*!
     * \brief Desconstructs the %BJob obejct.
     */
    ~BJob() override;
     ...
     * use the asynchronous interface of KJob instead.
     *
     ...
     *
     * The default for any %BJob is to automatically delete itself.
     *
     * \sa isAutoDelete()
     */
...
public Q_SLOTS:
     ...
     */
    bool kill(QHR::BJob::KillVerbosity verbosity = Quietly);
...
Q_SIGNALS:
     ...
     *
     * \note This is a private signal, it can’t be emitted directly by subclasses of %BJob, use emitResult() instead.
     *
     ...
     */
    void finished(QHR::BJob *job
#if !defined(B_DOXYGEN)
                  , QPrivateSignal
#endif
                  );
     ...
     *
     * \note This is a private signal, it can’t be emitted directly by subclasses of %BJob.
     *
     * \sa suspend(), doSuspend()
     *
     * \param job the job that emitted this signal
     */
    void suspended(QHR::BJob *job
#if !defined(B_DOXYGEN)
                  , QPrivateSignal
#endif
                  );
    ...
    /*!
     * \brief Emitted when the job is finished (except when killed with BJob::Quietly).
     *
     ...
     */
    void description(QHR::BJob *job, const QString &title,
     ...
     */
    void infoMessage(QHR::BJob *job, const QString &plain, const QString &rich = QString());
     ...
     */
    void warning(QHR::BJob *job, const QString &plain, const QString &rich = QString());
     ...
     *
     * \note This is a private signal, it shouldn’t be emitted directly by subclasses of %BJob, use setTotalAmount() instead.
     *
     * \param job       the job that emitted this signal
     * \param unit      the unit of the total amount
     * \param amount    the total amount
     */
    void totalAmount(QHR::BJob *job, BJob::Unit unit, qulonglong amount);
     ...
     * The unit of this amount is sent too. It can be emitted several times if the job manages several different units.
     *
     * \note This is a private signal, it shouldn’t be emitted directly by subclasses of %BJob, use setProcessedAmount() instead.
     *
     * \param job       the job that emitted this signal
     * \param unit      the unit of the processed amount
     * \param amount    the processed amount
     */
    void processedAmount(QHR::BJob *job, BJob::Unit unit, qulonglong amount);
     ...
     */
    void totalSize(QHR::BJob *job, qulonglong size);
     ...
     */
    void processedSize(QHR::BJob *job, qulonglong size);
     ...
     *
     * \note This is a private signal, it shouldn’t be emitted directly by subclasses of %BJob,
     * use emitPercent(), setPercent(), setTotalAmount() or setProcessedAmount() instead.
     *
     * \param job       the job that emitted this signal
     * \param percent   the percentage
     */
    void percent(QHR::BJob *job, unsigned long percent);
     ...
     * of %BJob, use emitSpeed() instead.
     *
     * \param job   the job that emitted this signal
     * \param speed the speed in bytes/s
     */
    void speed(QHR::BJob *job, unsigned long speed);
...
protected:
     ...
     *
     * You should define an (anonymous) enum of error codes, with values starting at BJob::UserDefinedError,
     * and use those. For example:
     *
     * \code
     * enum {
     *     InvalidFoo = BJob::UserDefinedError,
     *     BarNotFound
     * }
     * \endcode
     *
    ...
    BJobPrivate *const d_ptr;
    BJob(BJobPrivate &dd, QObject *parent = nullptr);

private:
    void finishJob(bool emitResult);

    Q_PRIVATE_SLOT(d_func(), void _k_speedTimeout());
    Q_DECLARE_PRIVATE(BJob)
    Q_DISABLE_COPY(BJob)
};

}

Q_DECLARE_METATYPE(QHR::BJob::Unit)
Q_DECLARE_OPERATORS_FOR_FLAGS(QHR::BJob::Capabilities)
